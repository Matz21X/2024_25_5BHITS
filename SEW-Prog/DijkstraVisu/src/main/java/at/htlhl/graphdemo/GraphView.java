    ...
    public GraphView(GraphControl graphControl) {
        ...
        smartGraphPanel.setOnContextMenuRequested(new EventHandler<ContextMenuEvent>() {
            ...
            public void handle(ContextMenuEvent contextMenuEvent) {
                ...
                SmartGraphVertexNode<VertexData> foundVertex =
                        findVertexAt(contextMenuEvent.getX(), contextMenuEvent.getY());

                if (foundVertex != null) {
                    System.out.println("Vertex clicked: " + foundVertex.getUnderlyingVertex());
    ...
    private ContextMenu buildContextMenu() {
        ...
        showDetailsItem.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent actionEvent) {
                SmartGraphVertexNode<VertexData> smartGraphVertexNode =
                        (SmartGraphVertexNode<VertexData>) contextMenu.getOwnerNode();
                Vertex<VertexData> vertex = smartGraphVertexNode.getUnderlyingVertex();

                Alert alert = new Alert(Alert.AlertType.CONFIRMATION);
                alert.initOwner(smartGraphPanel.getScene().getWindow());
                alert.setHeaderText("Vertex " + vertex.element());
                alert.setContentText("Detailed information to vertex " + vertex.element());
                alert.show();
            }
        });
